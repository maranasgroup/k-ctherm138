function [ k,dKdp,Revs,dRevsdp ] = calc_k_act( model,x )
...
for i = 1:nr
    ...
    if length(model.kinetic(i).act) > 0
        for jj = 0:length(model.kinetic(i).act)-1
            VR(length(VR) - jj) = p(1);
            V(length(VR)-jj) = 0;
            VR(length(VR)-jj-1) = p(1);%EL(length(VR)-jj-1,length(VR)-jj-1);
            %EL(length(VR)-jj-1,length(VR)-jj-1) = p(1);
            V(length(VR)-jj-1) = 0;
            dVRdp(length(VR)-jj*2,size(dVRdp,2)-jj) = 0;
            %dVRdp(length(VR)-jj*2-1,size(dVRdp,2)-jj) = 0;
        end
    end
    K = EL\(VR+V);
    ...
    if ~model.p.exch(i)
        ...
        Vf = dVfdp*p;
        Vf = Vf + vnet;
        if ~isempty(model.kinetic(i).act)
            kk = length(model.kinetic(i).act);
            for j = 1:kk
                Vf(ne(i) - kk + j) = Vf(ne(i) - kk + j) - vnet;
            end
        end
        D1 = prod(Vf);
        D2 = prod(Vr);
