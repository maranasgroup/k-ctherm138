function [r,W,J,vop,cs] = rescalc_reg(x,model,k)
...
v = zeros(nr,ncond);
sthresh = 1e-3;
[~,dkdp,revs,drevsdp] = calc_k_act(model,x);
...
for i = 1:ncond
    ...
    while ~done
        ...
        if max(abs(dx))<sthresh || (abs(vx(cind))<0.1)% && max(abs(dx))<1)
            ...
            % added the delow two lines to account for yeilds instead of
            % fluxes
            vsc = 100*vx/abs(vx(cind));
            vsc_store(:,i) = 100*vx/abs(vx(cind));
            dvscdk{i} = (100/abs(vx(cind)))*(dvdk{i}+vsc*dvdk{i}(cind,:)/100);
            %Css(:,i) = c(end,:)';
        %else
        %    ctr = ctr+1;
        %    c0 = css;
...
for i = 1:ncond
    ...
    %dvjdk = dvdk{i};
    vsc = 100*vj/abs(vj(cind));
    vj = vsc.*model.d.vpert(:,i);
    dvscdk{i} = (100/abs(vj(cind)))*(dvdk{i}+vsc*dvdk{i}(cind,:)/100);
    dvjdk = dvscdk{i};
