function [ xopt,fopt,fail,actcon,ropt,drdxopt,iter ] = lsqsolve( x,model,A,b,actcon )
...
nx = length(x);
tolx = 0.000001; % Termination tolerance on x
tolg = 0.000001; % Termination tolerance on gradient
...
if nargin < 7
    [r,W,drdx] = rescalc(x,model); % initial function evaluation
end
...
while ~done && f0 > 0
    ...
    if first
        ...
        if dfbase>1e-10
            df = 0.1;
            %df = max(dfbase*max(diag(H).*xsc.^2),1e6);
        else
            df = 0.1;
        ...
        if f < f0   % accept step
            ...
            if h == 1
                ...
                df = df*min(1,max(1-(2*Rz-1)^3,1/3));
            end
