...
% Check for metabolites included in reactions but missing from mets list
mettest = {};
for i = 1:length(rxnprop)
    mettest = [mettest;rxnprop(i).baseflx.reactant;rxnprop(i).baseflx.product];
    mettest = unique(mettest);
end
metsup = [metprop.metid]';

chk1 = ismember(metsup,mettest);
chk2 = ismember(mettest,metsup);

extmetsup = length(chk1)-sum(chk1);
disp([num2str(extmetsup),' extra metabolite(s) supplied in Metabolites tab']);
if extmetsup>0
    disp(metsup(~chk1));
end

newmets = length(chk2)-sum(chk2);
disp([num2str(newmets),' metabolite(s) missing from Metabolites tab']);
if newmets>0
    disp(mettest(~chk2));
end

if extmetsup>0 || newmets>0
    error('Error in Metabolites Tab. Please correct the list of metabolites and try again');
    %break
end
clear mettest metsup chk1 chk2 extmetsup newmets;
...
% Metabolite involvement matrix
[~,nr] = size(model.S);
mets = [model.metprop.metid]';
Inv = zeros(size(model.S));
for i = 1:nr
    m1 = [model.rxnprop(i).baseflx.reactant;model.rxnprop(i).baseflx.product];
    m1 = unique(m1);
    j = ismember(mets,m1);
    Inv(j,i) = 1;
end
